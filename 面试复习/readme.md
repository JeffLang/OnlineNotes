...
通常我们使用`addEventListener`注册事件，该函数的第三个参数可以是布尔值，也可以是对象。对于布尔值`useCapture`参数来说，该参数默认值为`false`。`useCapture`决定了注册的事件是捕获事件（`true`）还是冒泡事（`false`）件。对于对象参数来说，可以使用以下几个属性
...
总所周知`JavaScript`是门非阻塞单线程语言，因为在最初 JS 就是为了和浏览器交互而诞生的。如果 JS 是门多线程的语言话，我们在多个线程中处理 DOM 就可能会发生问题（一个线程中新加节点，另一个线程中删除节点），当然可以引入读写锁解决这个问题。
...
1. 执行同步代码，这属于宏任务（script算是第一个宏任务）
...
一个`timer`指定的时间并不是准确时间，而是在达到这个时间后尽快执行回调，可能会因为系统正在执行别的事物而延迟

下限的时间有一个范围：`[1, 2147483647]`，如果设定的时间不在这个范围，将被设置为 1。

**I/O**

I/O阶段会执行除了close事件、定时器和`setImmediate`的回调
...
- [《如何在页面上实现一个圆形的可点击区域？》](https://maizi93.github.io/2017/08/29/如何在页面上实现一个圆形的可点击区域？/)
...
### 6. `<keygen>` 是正确的 HTML5 标签吗？

`<keygen>` 是 HTML5 中的一个标签，但它的使用场景比较有限，主要用于生成加密密钥。然而，由于安全性和兼容性的考虑，许多现代浏览器已经不再支持或限制了 `<keygen>` 标签的使用。

`<keygen>` 标签在 HTML5 中被定义为一种表单控件，用于在客户端生成公钥/私钥对。它通常与加密操作相关，例如用于 SSL/TLS 证书的生成。

由于加密密钥的生成和管理是一个复杂的主题，并且涉及到安全和法规的要求，实际应用中通常使用专门的加密库和服务来处理密钥生成和管理，而不是直接使用 HTML 中的 `<keygen>` 标签。
